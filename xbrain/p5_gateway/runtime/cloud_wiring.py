...
OUTBOUND_PERIODS = {
    ...
    # *** 5.0 s 保活, NO 不能是 None. 与上面 geo/manifest 同一个错法, 同一个
    # 理由: 甲方协议说"连接/变化时发完整索引", 而我方拿不到 Qt 的 session
    # 建立信号(订阅是 Zenoh 内部的事, 不产生回调), 只能用周期近似"连接时".
    # 而 None 的实际后果比 geo/manifest 那条更彻底: 文件面今天无内容, 索引
    # [恒为空数组也就恒不变化], 于是"变化时发"这一支永远不触发 -- 一帧都发
    # 不出去. _file_index 的 docstring 自己写着"不发则与后端挂了不可区分",
    # 而 None 恰好就是不发. 2026-09-03 实测: 22 秒内该 key 零帧, 而 router
    # 上 Qt 对它的订阅是在的(clients=1).
    # 周期取 5.0 与 state/media 一致 -- 两者是同一类(低频全量快照, 无变化
    # 信号可依), 没有理由给不同的值.
    "data/file/index": 5.0,
