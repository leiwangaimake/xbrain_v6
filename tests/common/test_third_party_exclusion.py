...
def mods():
    """The six walkers, loaded once. charset_lint is imported under its own
    ...
    clock = _load("clock_scan", "scripts/lint/clock_scan.py")
    return charset, header, ecode, cxx, singular, clock
...
def _all_walked(mods):
    """Every path any of the six walkers would scan, as one list."""
    charset, header, ecode, cxx, singular, clock = mods
    ...
    walked += list(clock.iter_sources())
    return walked
...
def test_vendored_tree_is_declared_in_the_shared_list(mods):
    ...
    # The other five read charset_lint's list rather than keeping a copy.
    ...
    assert clock.charset_lint is charset


def test_no_walker_yields_a_vendored_file(mods):
    """The negative half: none of the six scan surfaces reaches the tree."""
