...
waypoints the frontend renderGeo needs -- v1.5 PLAN A: waypoints get geom:{lat,lon}
(so toXY projects them via enu_origin), routes pass their {lat,lon} points through
as recorded. Each check names the mutation it reddens (CLAUDE.md 3.3); the
load-bearing one is the geom shape (a bare [a,b] array renders in the wrong frame,
a missing lat/lon renders NOTHING -- toXY returns null).
"""
...
_PAYLOAD = {
    ...
    "waypoints": [{"geo_id": "w-1", "name": "东门岗亭",
                   "lat": 31.2003, "lon": 121.5007}],
    "routes": [{"geo_id": "r-1", "name": "营区日常",
                "points": [{"lat": 31.2010, "lon": 121.4990},
                           {"lat": 31.2003, "lon": 121.5007}]}],
...
def test_waypoint_reshaped_to_geom_object():
    routes, wps = geo_layers(_PAYLOAD)
    # geom is a {lat,lon} OBJECT -- toXY projects a {lat,lon} object via enu_origin.
    # MUTATION: emitting a bare [lat,lon] array makes toXY read it as ENU metres.
    assert wps[0]["geom"] == {"lat": 31.2003, "lon": 121.5007}
...
def test_route_points_and_kind():
    ...
    assert routes[0]["points"] == [{"lat": 31.2010, "lon": 121.4990},
                                   {"lat": 31.2003, "lon": 121.5007}]
...
def test_waypoint_without_coords_skipped():
    # A row missing lat/lon cannot be placed -> dropped, not rendered at (0,0).
