...
def one_object(
    *,
    ...
    # velocity_valid=False keeps the numbers as an unusable estimate (11 v2.3
    # row 2); unlocalized=True is the row-4 shape: geometry and velocity None.
    if unlocalized:
        obj = TrackedObject(
            track_id=track_id, class_name=class_name, class_id=0,
            confidence=confidence, semantic_status=semantic_status, footprint_xy=None,
            z_min=None, z_max=None, r_near=None, velocity_xy=None,
            velocity_frame=velocity_frame, velocity_valid=False,
            velocity_status="warming_up", stable_frames=stable_frames)
    else:
        obj = TrackedObject(
            track_id=track_id, class_name=class_name, class_id=0,
            ...
            velocity_frame=velocity_frame, velocity_valid=velocity_valid,
