...
| **CLD-5** | 待裁决 **E-2** `state/link` 的 L3（返航触发）映到 v2.0 哪个 `state` | 现状：抛错不发。映 `down` 让 Qt 显示离线而机器人仍在动；映 `degraded` 丢掉「已触发返航」 | `[GATED-DECISION]` |
| **CLD-6** | 待裁决 **E-3** `state/media` 保活频率（`11` 写 10 s，v2.0 要 5 s）· **E-4** 三个新 `E_*` 码是否进闭集 | E-3 不一致会让 Qt 每次误判端点掉线 | `[GATED-DECISION]` 见 `docs/MISSON/契约对撞_v2.0对11.md` E 节 |
