...
> 建立于 2026-08-12（P3 任务链路 PB1-8 收尾当日）；最近更新 2026-08-20（geo/teach 全链 + HMI 上行 W4/W2/W7 + P4 改发 §7.2 TaskCommand + 录制阻塞链定位）；2026-09-11 追加 §8（RNS 实机待办与 #20-15 挂起,用户裁决）。；2026-09-11 晚 §8.4（RNS SIL 第三轮收口 + 感知对账四轮后的遗留，用户裁决 #20-28 挂起）
...
| P1-2 | 速度门四段 f(d_free) + 迟滞、路径跟随、旋转门 RCG 全接 | ◐ **P7.2 已链**：f(d_free 前向扇区) · h（11 §3.6 HealthFactor 含 max_profile 降档与 3 s/10 s 阶梯）· i（i_fix × i_heading）· spec 上限 · 五道否决（estop/health/rtk/heading/感知失效）；路径跟随归 RNS。**已补链（2026-09-12/13）**：围栏裁剪（12 §7）· f 迟滞升档（12 §6.2/§6.7）；**未链**：RCG 旋转许可 · 三级限幅/加加速度 —— 见 §8.4 P7.3 |
...
## 8. RNS 实机待办与 #20-15 挂起（2026-09-11 追加,用户裁决「实机标定先跑」）

> 性质同本文件:对账表,设计真源在 `20` 各节。SIL 阶段(2026-09-07~11)已完成 RNS P0-P7 全栈 + 引导层 G1-G3 + 密集场验收 93.8%(16 案 15 过);以下为**实机依赖项**。

### 8.1 开工前置:实机标定

- [ ] **[sim] 参数全量复核**(`20` §12 逐键,流程 #20-4/#20-5):重点 `d_wall_m`/`front_stop_m`(依赖真实剖面质量)、`no_progress_m`/`max_follow_m`(场景尺度)、盾系(0.85 环/cap 斜率,依赖真实记忆撒点密度)。
- [ ] **真栈 P99 复测**(`20` §4A.6 G3):SIL 实测 compute P99 19.3ms(配额 150);真栈叠加感知/仲裁后按 P1 循环 P99 ≤ 60ms 红线验收。
- [ ] **剖面质量标定**:`h_block` 真实高度剖面、体素化误差形态 —— SIL 在契约层合成,跳过深度→体素→剖面管线(`11` §3.1B / `19`),这是仿真与实机第一差异面。

### 8.2 #20-15 · 贴墙深陷与「折线穿墙群」家族专项 `[GATED-HW]`(架构级,挂起至实机)

**是什么**(`20` §4A.7 撤退记录 · §4A.9 余留 · §4A.10 八版否证实录):贴墙态占比过高(密集场 ~50-75%);path 折线穿大墙群(漏斗几何)结构性长绕;热记忆连跑(正向完立即反向)可复现 `wall_closed_loop`/`wall_no_progress`(基线热三连 2/3)。

**为什么现在不做**(2026-09-11 夜实测):八版判据修复全部否证 —— 贴墙层微调呈**全局混沌敏感**,冷验收(16 案)与热连跑互斥,微调空间无全绿点。正解在架构层(贴墙对象判定的几何语义 / 记忆分层使用 / 离墙判据新语义),而三者参数靶心全依赖**真实感知噪声形态** —— SIL 纯净剖面上定靶必实机返工。

**实机后怎么开**:复现台架直接取用(`20` §4A.10 热连跑 fwd→rev→fwd2 + `scripts/sil/field_probe2.py` 16 案);八版否证表为负知识边界,🚫 不再重试表内判据补丁;验收 = 冷 16 案 ≥95% **且** 热三连 3/3 同时成立。

### 8.3 其余 RNS 实机挂账(编号项详见 `20` §15.1 台账)

- [ ] **#20-16** 有界后退行为接线(`backup_permitted` 判定就绪,`20` §7.5 RNS-N-13);
- [ ] **#20-19** 对准段三键悬空(需先裁决 goto 目标航向语义,`20` §2.5)`[GATED-DESIGN]`;
- [ ] **#20-20** RTK 三级接线(`classify` 就绪,等 ctx 真数据源,归 P7.1,`20` §3.2.1);
- [ ] **#20-21** `seg_recover_frames` 恢复迟滞(`20` §3.1.11);
- [ ] **#20-29** 目标在障碍内的兜底判定(P3 下单校验为主,`20` §4A.9)—— ★ 原借用 #20-22 号,该号 2026-09-11 已被 `20` §15 objects 年龄三档占用,改号 #20-29 并补登记 `20` §15;
- [ ] **path 离线判据裁决**(占比 15% 线 vs「每次离线事件后回线」,`20` §4A.9 —— 用户裁决项,可与 #20-15 合并处置)。

### 8.4 RNS SIL 第三轮收口后的挂起项（2026-09-11 晚追加,用户裁决「#20-28 现在不做,写入后续开发」）

> 结论快照:`20` v1.41 · 提交 `df26572`。三套台架(冷 16 案 / 随机 30 对 / 热双向 8 腿)改动前基线与改动后逐行逐腿同数,A/B 证明 B1~B4 代码批对 `rns-v2.0` 零行为变化;用户浏览器实测「RNS 没有一点问题」。本节是**不动 v2.0 行为**的前提下留给后续开发的账。

- [ ] **#20-28 · end-hop 子目标每拍重进入循环专项** `[GATED-HW]`(用户 2026-09-11 裁决挂起)。
  - 现象(`20` §4A.12 ④,热电池 `leg04_fwd` 实录):`_enter_wall` 以小障碍判定改走端跳子目标,子目标被采纳后下一拍即丢弃、再采纳,位姿冻结 400 s;v1.41 起由看门狗分支三兜成 ~120 s 有界失败 `watchdog_no_progress`(detail `escalations_exhausted: true`),**未解决到达**。
  - 复现:`python3 /opt/xbrain_v6/scripts/sil/hot_battery.py <tag> 8 4`(第 5 腿诊断,含状态直方图 / 轨迹 / 末尾审计)。
  - **开工触发条件(任一)**:① 实机或浏览器审计出现 `watchdog_no_progress` 且 `escalations_exhausted: true`;② P7.2 接线完成、实机感知进 RNS 后做整机 SIL 回归时,与 #20-27 合成一个专项。
...
- [x] **P7.2 · RNS 进仲裁 + 感知快照喂 tick** ✅ **2026-09-12 已落地**（提交 c91ad2c / b58e21e / 本批）：`nav/`（health_factor · route_intake · relmove_intake · host_gate · progress · report_map · mission_host · nav_tick）+ `sources/rns_avoid.py` + `runtime/nav_cfg.py`/`nav_wiring.py` + `configs/p1_motion.yaml` 四段取值 + 位姿桩 `scripts/dev/pose_stub.py`（sil_world 的 Zenoh 版：底盘/RTK/感知三合一）+ `scripts/dev/run_p72_e2e.sh`；RNS 包零改动。★ **浏览器 E2E（d8aa0d3）**：`scripts/sil/sil_server.py --mode e2e`（一键 `scripts/dev/run_sil_e2e.sh [--takeover]`，http://192.168.1.8:8890）—— 页面的「执行导航」发 `cmd/motion/route`、点地图发 `cmd/motion/relative_move`、重置发 `op=clear`，机器人由真 p1 的 `rt/motion/cmd_vel` 驱动；Zenoh 侧共用 `scripts/sil/zenoh_world.py`，`tests/sil/test_zenoh_world.py` 让桩报文经 p1 自身解析器往返。`--mode rns`（缺省）仍是进程内算法台架，行为不变。**边界（未做，登记于下）**：RCG/围栏/限幅未链、路径 loop/pingpong 单趟、纯旋转（Nav2 spin 委托）拒收 `E_CAPABILITY`、dev 栈 `cmd/motion/factor` 由位姿桩 `--grant` 代发（p2 Stage-D 发布器未建）、`path/relative_move.py` 旧六值 abort 集与 11 v2.0 七值不一致（P7.2 走 `nav/report_map.py`）。原文 ⇒ `[SW-NOW]`:`RnsSource` 接入 `p1_motion` 仲裁阶梯(行为源 `rns_avoid`,`12` §4.2c),`perception_in.latest(now)`(`perception_src/three_keys.py`,已在主接线声明)进 tick ctx —— 感知方「实机功能交付」阶段我方必须就位的对手件。
- [ ] **P7.3 · P1 输出链补齐**（P7.2 边界，`[SW-NOW]` 但可分批）：① RCG-1~4 旋转许可（`rotation/rcg.py` 已建，需侧向清距来源 —— 338Le 90° FOV 下环形掩膜依赖记忆栅格，与 20 §4.2 记忆域接口待定）；② 围栏裁剪 ✅ **2026-09-12 已接**（`fence/clip.py` 边距/v_fence/半空间投影 + `fence/episodes.py` 事件；NavTick 第 7 步 · `state/fence` geo/enforcement/allow 由每拍求值填写 · `p1_motion.yaml` fence 引用块；`gate.limiter=fence` 归因）；**SIL 围栏场景 ✅ 2026-09-12**（浏览器「电子围栏」工具连点右键闭合 → `POST /api/fence` → E2E 栈发 11 §9A.2 `cmd/fence`（共享 crc32 归一化，经 p1 接收器往返测试）；p1 的 `state/fence` 镜像到面板与地图三线：名义边界虚线 / 有效硬边界实线（内缩）/ 软带起点点线；脚本 `data/run/_fence_scn.sh`）。**实测**（活动区北边 y=5.7 切过默认路径，内缩 0.5）：`max_y = 5.21`（有效边界 5.20，名义 5.7 **从未越出**），`limiter=fence` 253 帧、`clipped` 256 帧、`outside` 0，任务 **ARRIVED 166.8 s**。★★ **发现 · RNS 不知道围栏**：机器人在有效边界上顶栏停滞约 70 s（RNS 候选正对栏外航点 (8.0, 5.9)，朝外分量被裁后切向分量≈0），直到 RNS 自行换招才滑到下一航点 —— 本期设计如此（12 §7「作用于最终速度指令」；`rns/candidate.py` 的 `fence_hazard` 门只是占位）。**待裁决**：是否把围栏几何喂给 RNS（候选级 `fence_hazard` / 记忆栅格标占）以避免顶栏停滞；以及 P3 侧 `cmd/fence` 真机联调。★★ **待裁决 · 内缩量 `inset` 三处口径不一**：11 §9A.6 ① 写 `inset = r_body + k_pos·cov_h + v·t_pos + e_sdf + v·σ_head·t_pred`（载体键 11 §9A.12 `fence.r_body_m/k_pos/t_pos_s/sigma_head_rad`，**configs 里没有这些键**）；11 §3.2.1 写 `clamp(k_sigma·cov_h + margin_ctrl, margin_min, margin_max)`（`margin_max` 需 `W_road`，12 §7.4 终审 F8 判其兜底方向错、未裁）；12 §7.4 / 12 §6.6 表① / `configs/common.fence.margin_by_fix` 写按 `fix_type` 查表（rtk_fixed 0.30 / rtk_float 1.00 / dgps·single 不内缩且 `fence_eval=unavailable`）。**代码现按第三种（12 §7.4，P1 详设 + 现有配置 + S-6 单调断言）实现**，`clip.inset_for_fix` 是唯一改动点；若裁决取 11 §9A.6 ①，需新增四个 P1 配置键 + `r_body` 与 `common.motion.free_space_corridor.r_body_m` 同源（12 C-4）。另：`speed_limit` 围栏「区内限速由 RNS 施加」（11 §9A.1A 待确认）本期不作边界；`inset_exceeds_geometry`（11 §9A.7 几何上限 `half_width_min`，「实现细节待验证」）本期未做。③ 三级限幅 + 加加速度（12 §8；`common.spec.max_accel_mps2` null 待 V-01）—— ★ **另需裁决 R-6**：12 §8.1 ② 写作对称的 `|v − v_prev| ≤ a_max·dt`，若减速也按 `max_accel_mps2`（sim 1.5）限斜坡，速度门按 `brake.a_mps2 = 2.5` 推导的 f 段界与围栏 `d_stop` 就会被追不上（f 降段不在 §6.5 否决表，字面上要走斜坡）；须定「② 只限加速 / 减速按 `spec.max_decel_mps2` / f 降段与 v_fence 豁免」后再做；④ f 迟滞升档 ✅ **2026-09-13 已接**（`gate/speed_gate.BandHysteresis` 逐段计时器：降段即时，升段需 `段下界 + d_up_margin_m` 持续 `speed_up_hold_s`；U54 顶段 3.5 m / 3 s，低两段按 12 §6.2 外推 2.3 / 1.75 m；掉线保留段位、计时清零；`p1_motion.yaml` `speed_gate.hysteresis` 私有块；`compute_gate` 改吃速度项）；⑤ ~~`state/arb/motion` + `event/*/arbitration`~~ ✅ 2026-09-12 复查批已发（`nav/arb_state.py`，11 §7A.5.1/7A.7 全字段，gen 仅胜出者变更 +1）；⑥ `teleop_cloud`(550) 对 RNS 的抢占（TR-RNS-1 第二支）：`_on_teleop` 现忽略 `source=cloud` 帧，云端遥控不会挂起 RNS —— ★ **门控在 12 §12 `teleop.cloud.enabled = null`（「本期是否放行云端遥控，待甲方接口评审纪要」，终审 S33「评审未过则该通路不存在」）**，纪要未到不做；⑦ 11 §7A.6.5 E-2 急停反查（`RobotState.estop_epoch`，待 quadruped）。★★★ **2026-09-17 实测把这条的范围放大了，且它现在挡着 RNS 的整条运动通路**：`11`:1722 逐字要求 `rt/motion/cmd_vel` 载荷「＋ **必填 `estop_epoch`**」，而 p1 现发的 body（`nav_wiring.py` `_publish_cmd_vel`）**没有这个字段**（全 `xbrain/` 内该字段只存在于 p5 云端急停链路）。按 `11` §3.0.1 的放松型规则（必填字段缺一即拒）与 `13` §7.1.0 `RX-3`，**quadruped 必然拒收 p1 的每一条 cmd_vel** ⇒ Tier 1 超时零速 ⇒ **RNS 的路接上也不会动**。⇒ **解开顺序**：① quadruped 发 `rt/chassis/state` 含 `estop_epoch`（本轮在做）→ ② p1 订它并在 `_publish_cmd_vel` 里回填 → ③ 整条通路才通。🚫 **不得在 quadruped 侧写宽松解析绕过** —— 缺省填 0 会让首次软急停后代际永不相等（零速锁死到重启），回填我方自己的值会让软急停保持形同虚设，两条都是 `CLAUDE.md` §3.2 的 fail-silent。
- [ ] **SIL/E2E · 到达秒数对回路相位敏感（2026-09-12 实测，不是缺陷，是判据说明）**：`run_p72_e2e.sh rev` 在同一提交上可复现（8e3db3c 两次 88.0 s / 1759 帧），但把位姿桩晚启 25 ms 就变 144.0 s（贴墙混沌分支，20 v1.32）；围栏接线后的 140.5 s 即此效应。⇒ **主机层零回归判据 = 逐拍等价测试**（`tests/p1_motion/nav/test_nav_tick_fence.py::test_no_fence_stage_is_a_pass_through_over_a_whole_drive`）+ E2E 的 verdict / tick errors / P99，🚫 不拿到达秒数当回归信号。A/B 工具：`data/run/_ab_e2e.sh <ref> rev`、`_ab_phase.sh <ref> <delay_s> rev`（gitignored）。
- [x] **P1 · 信封 ts/mono 单位偏离（既有）** ✅ 2026-09-13 已改：`stamp_envelope` 改走共享 `Envelope`/`encode`（秒 float64；无 boot 时 mono 一并省略，CLK-C4）；三个消费者核对过均不读信封 ts/mono（p4 `_fresh_data` 按缓存 RX 龄期、p5 `pose_group` 只读 data、p2 `refresh_health` 不看信封）。原文 ⇒ `path/gnss_pose.stamp_envelope` 对 `state/pose` / `state/clock` / `state/fence` 戳【毫秒整数】，11 §3.0 逐字为【秒 float64】；P7.2 新 key（cmd_vel / path_progress / relative_move/status / state/arb/motion）已改走 `xbrain.common.envelope.encode`（秒）。旧三键改秒须与 p5 HMI 读者（`data_readers.pose_group`）同批核对。
- [ ] **P1 · `stop_reason` 归因未上线（既有）**：`CtrlLoop` 把 SAFE_STOP 记作 `soft_estop`，健康度否决的零速没有专属 stop_reason 值（11 STOP_REASON 八值无 health）；该字段随 PS-4（`state/pose.motion` 与 `cmd_vel.gate` 同构体）落地时一并裁定。
- [ ] **感知第六轮 · `depth_quality` 去留**：`11` §3.1B.2 用途表列有 `depth_quality`（远处深度不可信时降权），但样例 / DTO / 解析器 / RNS 均无此字段（`perception-rns-reply-20260912-r6.md` §三）。下一轮定：进 schema（并给 RNS 消费规则）或删表行。
- [ ] **感知第五轮 · 回退依据链两项待负责人裁定（`perception-rns-reply-20260912-r5.md` §五）**：D1 perception 订 `state/chassis_motion`（`height_m`）进 `11` §1.1.6 白名单（`19` PD-19）；D2 标定层新键 `calib.body_height_m` ＋ `19` §8.2 `fallback_align_err_m` 安装实测。裁定前回退 FREE 能力按收窄口径视为未完成、未验收（保守方向，RNS 零新逻辑）。
- [ ] **配置 · 11 §2.4.7 QoS 绑定表 vs 冻结线断言 F（既有矛盾）**：契约表的 `rt/** -> Q3_cmd` 兜底行被 F 的 `a5_block_on_rt` 拒绝（Q3_cmd 是 block）；夹具/`common_sim.yaml` 一直用 4 行替代表，所以 `tests/common/zenoh/test_qos_resolve.py::test_fixture_bindings_are_verbatim` 长期红。要么改契约表（rt 兜底走 Q1/Q2），要么 F 放行显式的 rt 兜底行 —— 需裁决后两处同改。
- [x] **配置变体制（用户 2026-09-12 裁定，`10` §5.4.7）** ✅ 已落地：`X_sim.yaml` 同目录覆盖层（只填 null）、冻结线 `--variant sim`、`MANIFEST.config_variant`、`safety/` 无变体、注册表按基名；夹具 `NULL_OVERRIDES`/`P4_L6_FILL` 清空，值迁入 `common_sim.yaml` / `models/m20s_sim.yaml` / `p4_agent_sim.yaml` / `sites/sim.yaml` / `calib/dev.yaml`；dev 快照 `v_nom_mps` 由被盖的 1.5 恢复为真值 2.0。顺带修了真 `common.yaml` 的 `enu_origin`：整节 null 改三叶子 null 占位（断言 A 只豁免三条叶子路径，原形态即使站点文件齐全也过不了冻结线）。
- [x] **p2 · `cmd/motion/factor` 发布器** ✅ 2026-09-12 已建：与 `health/summary` 同一聚合、同一 1 Hz 块发布（11 §3.6 五字段，`reason` = 主导项），P1 解析器往返测试。★★ **但生产上它现在恒发 `allow_motion=false`**：`cam_rgbd` 项没有数据源 —— 14 §8.1 让它看 `state/targets` 帧率/龄期，而 `state/targets` 在 19 §16 PCC-9 是「停车场，本期不实现」，11 §1.1.6 又刻意不让 p2 订 `rt/perception/status`（11 T-27 / #20-10 已登记该矛盾）。**待用户裁决相机健康来源**：(a) 改 11 §1.1.6 让 p2 订 `rt/perception/status`（1 Hz 心跳，已有 fps/degraded_reasons/extrinsic）；(b) p1 作为感知跨面点把 `StatusMsg` 摘要镜像到通用面新 key `state/perception`（同 `state/pose` 范式，需 11 新增 P1 发布行 + p2 订阅行）；(c) 让感知方发 `state/targets`（与 PCC-9 冲突）。建议 (b)。裁决前 dev/E2E 栈继续用位姿桩 `--grant` 代发，且不要与真 p2 同时跑（两发布者会在 p1 侧交替）。
...
| **R-4a**（2026-09-13 SIL 实录，禁入区实验） | **路线未经围栏校验就下发**：用户在西南那面墙 (−4.3,−2.3)–(−7.3,−6.7) 周围画了一块 `forbid`，反向路线第 18/19/20 号航点 (−6.0,−4.4)/(−6.5,−3.3)/(−6.8,−2.1) 全在禁入区内，第 21 号距边界 1.09 m；裁剪按 12 §7 只作用于最终速度，机器人从未入区，但三个不可达航点照样下发。真机链路里这一步归 **P3 围栏前置校验**（11 §9A / 15），SIL 的 sil_server 直发 `cmd/motion/route` 没有这一层 | 无（登记事实） | P3 前置校验落地时把「航点落入 `forbid` / 落出 `allow`（含内缩）」列为拒绝或改线条件；SIL 可在 `/api/nav/execute` 加同款预检 |
| **R-4b** | **RNS 不知围栏的最坏形态**：RNS 只看见真墙与开阔地，按贴墙逻辑绕墙南端追航点 18，绕行路径正落在禁入区，被裁剪吃掉 ⇒ 13:26:45–13:32:31 `wall_follow` 5 分 46 秒毫无进展，机器人最终贴在有效边界 (−1.03,−9.12)（d_nom 0.50 / d_eff −0.001，朝内分量为零）。与 y=5.7 活动区实验的「顶栏 70 s 后自行换招」不同，这次整段路线不可能完成 | 同 R-4 | 同 R-4：围栏几何进 RNS（候选级 `fence_hazard` / 记忆栅格标占），否则贴墙逻辑会一直把围栏当成可绕的空地 |
| **R-4c** | **失败来得慢、归因还错**：停滞由围栏造成，却等 RNS 的贴墙预算判死（`wall_no_progress`），按 12 §4.2c.6 映射成 `obstacle`，操作员看不出是围栏；p1 侧明明有 `limiter=fence` 帧（最后 25 s），但 12 §4.2c.6 只有 `watchdog_no_progress` 一行按「argmax 限速源」归到 `fence`，`wall_no_progress` / `wall_budget` 两行固定归 `obstacle` | 现行代码 = `nav/report_map.py` 按 12 §4.2c.6 表映射 | 裁决：是否把「失败前 N s 内 `fence` 为主导限速项」作为统一的 `fence` 归因条件（覆盖 wall_* 两行），以及是否加围栏专属的快速失败（顶栏零速超过 T 即 `fence` 中止，不等贴墙预算）—— 改 12 §4.2c.6 后再动代码 |
...
### 8.9 ★★★ 2026-09-18 · quadruped 通道三接线与 `T-ODOM-1` 验收（提交信息补记）

> ⚠️★★★ **为什么单开这一节**：本批代码被**另一个会话**的 `git add -A` 卷进了提交 `976a9f4`，
> 那条提交的标题是「docs: NEXT Orin NX 装机订正 - ROS2 已装, TensorRT 归感知侧」，
> 而它实际带进去的是 **12 个文件 711 行**的 quadruped 通道三接线与 `T-ODOM-1` 仪器。
> ★ 用户 2026-09-18 裁决：**不动历史**（另一会话可能仍在工作，`reset`/`rebase` 会毁掉它 —— CLAUDE.md §8.3 有血），
> 补本节把设计理由与验收证据记清楚。★ 代码本身完整，测试与变异体全过。
> ★★ 同时登记一条协作事实：**同一个 sshfs 仓库上并行跑两个会话时，`git add -A` 会互相卷入**
> ⇒ 🚫 今后提交一律**逐文件 `git add`**，不用 `-A` / `-a`。
>
> ⚠️★★★ **2026-09-18 补强 —— 上面那条不够，仓库真的坏过一次**：`tick_stats.h` 与
> `test_tick_stats.cc` **从未进入版本控制**（`git log --all` 零命中），而引用它们的
> `CMakeLists.txt`（3 处）、`rt_runtime.h`（3 处）、`cxx_mutants.py`（14 处）**都已提交并推送**
> ⇒ **任何人 clone 下来都编译不过**，实证方式是 `git archive HEAD` 展开后文件 MISSING。
> ★ **成因链**：① 我新建文件尚未 `add`；② 另一会话 `add -A` 把它们卷进它的提交；
> ③ 我据此以为已入库；④ 该会话改写历史把我的文件**剥离**（这一步本身是对的）；
> ⑤ 剥离出来的文件回到工作区**无人提交**；⑥ 而引用方已被我推送。
> ★★★ **真正的教训**：**逐文件 `add` 只在「我知道有哪些文件」时成立**。新建文件被别人的
> `-A` 卷走之后，我的 `git status` 是干净的 —— 而 `git status` 干净恰恰是我判断"都提交了"
> 的依据，于是那两个文件从视野里消失。⇒ **新增文件落地后当场 `git add`**，🚫 不得指望
> 后续某一次 `add` 顺手带上。
> ★ **可执行判据**：`git archive HEAD` 展开后，**引用与被引用必须同时在树里**。

**这一批做了什么**

| # | 改动 | 为什么 |
|---|---|---|
| ① | `main.cc` 建 `Uplink` ＋ `RtRuntime::SetOdomSink`，`PubLoop` 每拍从无锁槽取 `OdomSample` 发 `Odometry` ＋ TF | ★★★ 关闭 `13` `ASM-4` 登记的缺口。**接线前 `Uplink::Publish` 在生产代码里零调用点** —— 两端（`odom_slot_` 与 `quadruped_uplink`）都在、编译全绿、21 个测试全过，缺口只存在于 ASM-4 那一行字里。★ 那一行写着「这是缺口，不是设计，写在这里是为了不被后来者当成已完成」—— **它救了这次** |
| ② | 注入 `OdomSink` 而不是让 `rt_runtime` 直调 `Uplink` | ★ `quadruped_rt` 必须保持零 rclcpp 依赖（§5.3：`chassis_relay` 在急停链路上，共享面不得引 ROS）。直调会把 rclcpp 拖进 RT 面库 |
| ③ | 新增 `include/quadruped/tick_stats.h`：固定桶直方图（0.1 ms/桶，零分配 O(1)） | ★ `T-ODOM-1` 的判据是**百分位**，而均值产生不了百分位 —— 一个 99% 守住 10 ms、其余卡 200 ms 的循环，均值看起来很正常，`T-ODOM-1` 直接不及格。★ max **精确保存不量化**（判据直接比 max）；百分位取桶上界（向不通过方向取整）并 **clamp 到 max** |
| ④ | `OdomSample` 新增 `stamp_wall_s`，由 `ctrl` 在积分时刻打戳 | ★★★ 修一个真缺陷：`13` §9.1 `rt_pub` 行逐字「**时间戳取自样本、不取自发布时刻** ⇒ 发布晚了是到得晚，不是数据错」，而第一版在 `PubLoop` 里读 `system_clock::now()`。★ 后果**方向是反的**：丢了调度时隙的循环会把 40 ms 前的位姿**重新标成当前**，于是 `V-69` 换掉的那点抖动在下游**不再可见** —— 调度越差，测出来越好看 |

**仪器自身抓到的两个 bug（都在第一次运行就红）**

> ① `PercentileMs` 的 `++want` **不是 ceiling** 而是无条件加一：`p=0.99, n=100` 时精确秩是 99.0，那种写法返回 100，**把最慢的样本报成了 P99**。★ 它长得像 ceiling，且在 `p*n` 非整数时也能过 —— 用 `n=101`（`0.99*n = 99.99`）的用例才分得开两种取整。
> ② 首次真机跑打出 `p99=10.10 max=10.06` —— **百分位大于最大值**，算术上不可能。桶上界取整没有 clamp 到真实 max。

**`T-ODOM-1` 验收（2026-09-18，底盘上电，全程 `axis frames sent=0`）**

> `samples=64004`（640 s × 100 Hz）· **`p99 = 10.30 ms`**（判据 ≤ 12）· **`max = 15.80 ms`**（判据 ≤ 20）· `overflow = 0` · `odom sent = 63926`
> ★ 负载：6 个普通优先级 CPU burner（8 核），`load average` 0.3 → **6.37 / 5.88**；空载对照 `p99=10.10 max=10.06` ⇒ 负载确实进入了测量。
> ⇒ **`V-69` 收口**：把发布从 `SCHED_FIFO` 的 `ctrl` 挪到普通优先级的 `rt_pub`，这笔交易在本机成立，余量 P99 14% / max 21%。
> ⚠️ 口径：命令行跑、非 systemd 单元（无 `LimitRTPRIO` / `CPUAffinity`）⇒ 对本项是**保守方向**，详见 `13` §11.1。

**变异体**：42 个逐条实测跑红（`process` 33 ＋ `tick_stats` 9），零存活。
...
| ② ✅ **已关闭 2026-09-18** | ~~**`MANIFEST` 字段：文档有而实现不产出**~~ | ★ 用户裁决「补上」⇒ 四个字段（`gen_ts` · `robot_id` · `site_id` · `processes[*].refs`）已全部实现，`config_variant` 已补进 `10` §5.4.4 样例块。★ `refs` 在**展开前的 L6 源树**上数（展开后引用已不存在）；`robot_id`/`site_id` 取**解析后**的值（L5 可覆盖，且 `layers[]` 的 L4 行按同一值选文件）；`gen_ts` 是唯一允许两次 freeze 之间不同的顶层字段，`--check` 有意不比它。★ 变异体 13 个实测跑红 | — |
...
**★★★ 第二轮（2026-09-15，用户在 `.104` 上授权了本机公钥，`ssh user@10.21.33.104` 免密；`.103` / `.106` 尚未授权）—— 只读侦察结果：**

| # | 实测事实 | 意义 |
|---|---|---|
| 5 | **主机角色以机器上的 `/var/opt/robot/conf/robot_hardware_info.toml` 为准**：`model=CA9C`（M20S）`type=STD`，`aos_id=103 · gos_id=104 · vos_id=105 · nos_id=106`；`.104` 的 `/opt/robot/os_type = CA9C_GOS` | ★ 《软件开发指南》设备状态表的 `SocId`（NOS=104 / GOS=106）**与机器相反**；集成方案「106/NOS」是对的。13 册 / 11 册引用 SocId 处待订正 |
| 6 | **`robot_server.service` 在 AOS（`.103`）**，由 `nodectl`（`/var/opt/robot/conf/nodectl/nodectl_CA9C.yaml`）编排，`start_order 30`；同机还有 `motion_master` · `battery_node` · `deviceCtrl_node` · `gnss_node` · `charge_manager` · `reflective_column` · `audio_node` · `mediamtx` · `usb_cam` · `height_map_nav` · `lio-perception` · `handler`。NOS（`.106`）跑 `imu_node`（yesense IMU）· `localization` · `planner` · `global_planner` · `traversability_estimation` · `accumulate_cloud`；GOS（`.104`）跑 `rsdriver`（两台 RS-Airy 合并为 `/LIDAR/POINTS`）· `hsLidar` · `deviceCtrl_node` · `system_monitor` | ★ 指南「附录 2」缺失的服务清单由此补齐（V-57 的数据源）；关加密要在 **AOS** 上改 `robot_server` 配置，需 `.103` 授权 |
| 7 | **DDS「仅主机传输」的机理**：`/opt/robot/fastdds.xml` 默认 `interfaceWhiteList` 只有 `127.0.0.1`；`setup_ros2.sh` 按主机末段把 `10.21.33.103 / 10.21.31.104 / 10.21.33.106` 写进白名单（需 root）。机器人节点走 DrDDS（`/var/opt/robot/conf/drddslib/dds_topic_scope.conf`，未列话题默认 `multi` = 走物理网口），故外部主机**可直接订阅** | ★★ V-43 定案：不是「不能外接」，是**订阅方**要把自己的网口写进 FastDDS 白名单；`/LIDAR/POINTS` 默认 scope multi |
| 8 | 用临时白名单副本（`/tmp`，不动机器文件）从 GOS 实测：`/IMU` **201 Hz**（`frame_id` 为空）· `/IMU_103` 201 Hz（AOS 上第二个 IMU）· `/MOTION_INFO` **20 Hz** · `/JOINTS_DATA` 50 Hz · `/BATTERY_DATA` ~1 Hz · `/LIDAR/POINTS` **0 Hz**（`/LIDAR/STATUS` 两台 `is_runing=false`，趴卧空闲时雷达未上电）；`/MOTION_INFO`：`state=0 · gait=0 · height=0.121 m · remain_mile 6.63 km`；`/HES_STATUS=0`；电池 73.9 V / 50 % / 28~29 ℃；`/CHARGE_STATUS` 空闲。QoS：`/IMU` `/LIDAR/POINTS` BEST_EFFORT，`/MOTION_INFO` `/NAV_CMD` `/BATTERY_DATA` RELIABLE，`/HES_STATUS` TRANSIENT_LOCAL | ★ 13 §4 的 IMU 200 Hz / 运控 10 Hz 前提：DDS 侧 `/MOTION_INFO` 是 20 Hz（指南一致）；★ C30-4 说点云 `frame_id="rslidar"`，`rslidar/config.yaml` 写的是 **`lidar_link`**，待雷达上电后实测；两台 RS-Airy 安装位 `x=±0.32028 z=−0.013 pitch=−1.5708`（与硬件 §1.10 一致），组播 `224.10.10.201/202`，MSOP 6691/6692 |
| 9 | **资料到手**（`data/run/chassis/`，gitignored）：`drdds/msg/*`（`/opt/ros/jazzy/share/drdds`，含 MotionInfo/MotionState/Gait/NavCmd/FaultEvent 等 60 余型）· `fault_rules_CA9C.toml`（**387 条**故障规则，含阈值与迟滞）· 三份探针脚本 | ★ 集成方案 Q1（drdds）与 Q6（完整故障码表）**已由机器本身回答**；`FaultEvent.msg` 头注给出码段：`0x80xx` 关节 · `0x81xx` 电池 · `0x82xx` 主板 · `0x83xx` 外设 · `0x84xx` 运控 · `0x85xx` 充电 · `0x86xx` 系统监控 |
| 10 | 网络：GOS `eth0=10.21.33.104`，`eth1=10.21.31.104`（**无载波**）；静态路由 `10.21.41.0/24 via 10.21.33.103`（云深处远程维护段，`.104` 上次登录来自 `10.21.41.8`）；两台雷达在 `.201/.202`；`.11` 未识别 | ★ 底盘第 ③ 口「网口+PPS+TX」大概率就是 `10.21.31.x` 传感器网（PPS 供雷达授时），Orin 两个有线口正好一控制网一传感网 |

**★★ 解卡条件（二选一，均需用户/云深处）**：(a) 云深处签发的客户端证书 + 私钥 + CA（`Project CA` 链）落到 `configs/secrets/chassis_tls/`（权限 0600，不进 git）；(b) 登录 AOS 改 `robotserve` 配置关闭加密（指南 §1.1.2「需联系技术支持」）。★ 凭证到位后的验收顺序：心跳 → 四路上报 → `MotionParam=1` 站立 / `=4` 趴下（用户 2026-09-14 允许，🚫 不发轴指令）。

**★★★ 第三轮（2026-09-14 23:37～23:45 CST；`.103/.104/.106` 三台均已免密；用户批准「关加密 → 重启 robot_server → 明文探针 → 站立/趴下」四步）—— 卡点 (b) 已解；全部命令、日志、上报样帧与**还原台账**在 `docs/QUADRUPED/M20S_底盘接入与探测实录_2026-09-15.md`（§4.4 / §8 / §9）：**

| # | 实测事实 | 意义 |
|---|---|---|
| 11 | AOS `user_conf.toml` 追加 `[Network] enableTls = false`（出厂 108 B md5 `c0c9cd85…` → 137 B `de7105d2…`，出厂副本在 `data/run/chassis/factory_backup/`）；从 GOS 用 rclpy 向 `/NODECTL_CMD_103` 发 `NodeCtlCmd{restart, robot_server.service}` 即重启 drsec 容器内的 robot_server（pid 8958 → 350415）；日志 `port: 30003, mode: plain` / `port: 30004, mode: plain` | ★ **明文模式仍用 30003/30004**，不是老手册的 30000/30001；⚠️ **当前机器 ≠ 出厂**，还原 = 覆盖备份 + 再发一次 restart（实录 §8 C-4/C-5） |
| 12 | 明文心跳（16 B 头 + `PatrolDevice` JSON，hex32 按十进制）即得应答与五路上报：运动 `0x00100001` **10 Hz**（`MotionStatus` + 16 关节角）· 电池/CPU `0x00100002` 2 Hz · `BasicStatus` 2 Hz · `ErrorList` 2 Hz · 定位/导航 `0x00100003` 1 Hz；UDP 8 s 共 144 帧 | 13 §2.2 / §4 上报节拍定案；`CPU.AOS.SocId "103" / NOS.SocId "106"` 再证指南 SocId 表反 |
| 13 | `MotionParam=1`：ack 0 → `MotionState 1`（+0.1 s）→ **固件自动 `站立状态 --> RL模式`，稳态 `17 / Gait 0x1001`**（+3 s）；`MotionParam=4`：→ `4`（+0.3 s）→ **自动回 `0`**（+4 s）；终态 `Height 0.1215 m` 与动手前一致 | ★ 13 §5.2 与 MS-1 读回比对：`stand` 的期望三元组是 `{…, 17, 0x1001}`，`lie` 终态是 0；🚫 本轮未发任何轴指令 |
| 14 | 心跳超时 3 s；每次连接建立 / 断开触发语音（id 4 / id 3）+ 灯效；心跳丢失**不**改变运动状态 | 13 §2 心跳节拍 / 11 CHS 健康判据；正式接入须常驻心跳，🚫 不要用短连探针轮询 |
