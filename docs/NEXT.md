...
- [ ] **P1 · 信封 ts/mono 单位偏离（既有）**：`path/gnss_pose.stamp_envelope` 对 `state/pose` / `state/clock` / `state/fence` 戳【毫秒整数】，11 §3.0 逐字为【秒 float64】；P7.2 新 key（cmd_vel / path_progress / relative_move/status / state/arb/motion）已改走 `xbrain.common.envelope.encode`（秒）。旧三键改秒须与 p5 HMI 读者（`data_readers.pose_group`）同批核对。
