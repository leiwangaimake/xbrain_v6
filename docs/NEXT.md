...
| ★ SW-12 | **事件存证链路落地:record.db DAO + 7 阶段 pipeline 接线 + backfill 断网补发** | 中-高 | ★★★ **2026-08-17 审计发现** —— 事件"到 HMI 实时"已通(W2:`event/**`→内存环 50→`/api/events`+WS),但"**存证→断网补发→上云**"整条基本没落地。现状四缺口:① **record.db 无 DAO** —— P3 三库(task/fence/geo)有完整 DAO,**P5 的 record.db 一个没写**(`p5_gateway/` 下无 `dao/`/`persistence/` 目录,全仓只 `event/schema.py` 沾边);② `common.db.record_db` 路径 **null**(`configs/common.yaml`,归 SW-6);③ **7 阶段 `pipeline.py` 写好了但全仓无 import/调用** —— 运行的 voice-loop MVP(`main_wiring.py`)只做内存环喂 HMI,不写库不上云,`pipeline.py` 是躺着的组件;④ 云上行(第 5 步 cloud)卡 **SW-4**(甲方)。★ **设计已完整**:`17` §3 七阶段(schema→dedupe→level→**persist**→cloud→hmi→delivered)+ record.db `events` 表结构(`backfill_batch`/`last_backfill_at`,17 §3.2)+ `11` §6.2 事件表 + backfill 限速(§10:20 eps,`alarm:normal=4:1`)。★★ **为何要紧**:`pipeline.py` 自注"**persist 必须在 cloud 之前**",⇒ **没 record.db,断网补发与可靠上云立不住**,事件飘过 HMI 即丢,报警证据链(事件行 + 媒体交付行,DP-2)也无处存。★ 依赖:SW-6(record.db 路径落值)· SW-4(甲方云上行)· `15` §9(`aiosqlite` + `BEGIN IMMEDIATE` 事务,🚫 禁同步 sqlite3)。★★ **2026-08-17 增 · 设备掉线事件产生方未接线**:`payload`/`ptz`/`voice` 三类已在 `11` §6.2 定义 `device_offline`/`device_online` 子事件(补齐 payload/MIC/PTZ 三个掉线缺口),但**产生方尚未 emit** —— payload-service(8519/8529 socket down / WS 1011)· audio_io(USB `hw:0,0` 消失,对齐 `AsrGate.device_fault`)· ptz 驱动(ONVIF 不可达)检测到掉线/恢复时须 publish `event/{alarm\|info}/{payload\|ptz\|voice}`;随本条 pipeline 接线一并落地。★ 注:即便接线,事件是**先 persist 再 forward**(store-and-forward),live 连通才实时、断链走 record.db backfill(非无条件"立刻实时") |
