...
> ⚠️★★★ **2026-09-08 冻结留档**：本文件已交付并获感知方逐条答复（其答复抓到本文多处实错——探针数据当主线、凸包误判已有、PROF-1 伪代码反例等）。**自此不再就地修改**；全部技术更正已落三册真源（`11` v1.9 / `19` v1.2 / `20` v1.7），双方后续以 [perception-rns-reply-20260908](perception-rns-reply-20260908.md) 为准。
>
...
| ⚠️ **本期范围（2026-09-08）** | **你方本期只做三件事**：① 三条 key（§二 ＋ §四的算法）② 推理 20 Hz（§一A）③ 落地顺序四步（§七：标定 → 延迟 → ProfileMsg → 坐标正名）。<br>**跟随（follow_target）与 re-ID（§一B）本期不实现、不排期** —— §一B 只是背景与立项储备，Q-10 不用现在答 |
...
**依据**：`11` §3.1B（接口契约 · 唯一真源）· `20` §3.1（RNS 消费侧语义与不变量）· `19` v1.0（perception 详细设计 · 落地版 —— 本文的算法与工作项在该册 §3 / §15 有可编码规格）
