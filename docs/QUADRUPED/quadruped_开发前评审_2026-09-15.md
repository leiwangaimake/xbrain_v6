...
> · **B1 已落**（2026-09-15）：CHS-A 编解码与分帧（FR-1/2/3/5），金标取自实录抓包，变异体清单固化在 `scripts/ci/cxx_mutants.py`。
> · ⚠️ **B1 不含 socket** —— `TCP_NODELAY`（FR-5 前半）、端点探测、心跳搭车都在 B2；`13` §11.1 `T-CHS-3` 只做了**离线预验**，未满足。
> · **B2 已落**（2026-09-15）：配置数据路径补六键（`reconnect_backoff_s` 等原本 yaml 里有、C++ 侧读不到）· 上报解析（开放集三禁令、`chs:` 前缀 CF-1～CF-5、`Sleep`）· 会话状态机（端点探测 ＋ TLS-4、上下行失效、退避、`13` §7.5 响应码、TX-5 心跳搭车）。
> · ⚠️ **B2 仍不含真套接字** —— 会话注入 `dial`/`hangup`/`credentials` 三个可调用对象，策略全部离线可测；POSIX connect/send/recv、`TCP_NODELAY`、非阻塞握手未写，随 B9 台架一并做。
> · ★ **B2 期间的实测发现**：静止时 `Gait` 上报 `0`，`13` §5.3 步态表无此值 ⇒ 已登记 `13` **V-66**（v1.6），按开放集处置。
