...
**★★★ 提交前必须现场跑，🚫 不许凭记忆**（2026-09-03 实测：以下两条我都是「知道规则、
仍然违反」，因为把判据留到了提交之后）：

```bash
python3 scripts/lint/charset_lint.py | grep "in comments"   # §2.2，必须 0
python3 -m pytest tests/repo/test_vcs_hygiene.py -q          # §8.1 格式，必须绿
```

⚠️ **实测两次**：① 写了 3 个 `★` 在注释里（§2.2 只在 `docs/*.md` 放行），
lint 输出**打印了但没看**就推了；② scope 写成 `p3+p5:`，而我前一天刚指出过同样的历史提交。
⇒ **把判据跑在提交之前**，别把输出打出来当作已经看过。
...
| 把 lint 输出**打印出来但没看**就提交（§2.2 的 `★` / §8.1 的 `a+b` scope） | §8.1 · 2026-09-03 两次 | 判据跑在**提交之前**，看到 0 才 `git commit` |
